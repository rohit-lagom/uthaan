          ...
          {stakeholders.map((stakeholder, index) => {
            ...
            return (
              <motion.div
                ...
                className={`bg-white/60 backdrop-blur-xl border ${colorClasses.border} rounded-xl p-4 flex items-center gap-4 hover:bg-white/80 ${colorClasses.hoverBorder} transition-all duration-300 shadow-xl`}
              >
                <div className={`${colorClasses.bg} p-2 rounded-xl ${colorClasses.hoverBg} transition-colors`}>
                  <stakeholder.icon className={`w-6 h-6 ${colorClasses.text}`} />
                </div>
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-slate-900">{stakeholder.title}</h3>
                  <p className="text-slate-600 mt-2 text-sm leading-relaxed">{stakeholder.benefit}</p>
                </div>
              </motion.div>
            );
          })}
            ...
            return (
              ...
              >
                <div className={`bg-white/60 backdrop-blur-xl border ${colorClasses.border} rounded-3xl p-8 h-full hover:bg-white/80 ${colorClasses.hoverBorder} transition-all duration-300 transform hover:scale-105 shadow-2xl hover:shadow-3xl`}>
                  ...
                  <p className="text-slate-600 leading-relaxed">{stakeholder.benefit}</p>
                </div>
              </motion.div>
            );
          })}
